...
int main(){
    //Worst Case
    int rdmArr_Worst[10] = {3,2,4,7,6,9,1,5,8,0};
    size_t worstSize = sizeof(rdmArr_Worst)/sizeof(int);


    //Average Case
    int rdmArr_Average[10] = {0,1,2,3,5,4,6,8,7,9};
    ...
    //Best Case
    int rdmArr_Best[10] = {1,2,3,4,5,8,7,9,6,0};
    ...
    for(int i = 0; i<arraySize; i++){
        for(int j=i+1; j<arraySize; j++){
            if(rdmArr[i]>rdmArr[j]){
                int placeHolder = rdmArr[j];
                rdmArr[j] = rdmArr[i]; 
                rdmArr[i] = placeHolder;
            }
        }
    }
    return rdmArr;
